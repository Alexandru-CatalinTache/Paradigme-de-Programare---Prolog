...
intersectie(integ(H, W, Lista, Voc), I1, Poz1, I2, Poz2) :- lungime_spatiu(integ(H, W, Lista, Voc), I1, Lung1), lungime_spatiu(integ(H, W, Lista, Voc), I2, Lung2),
                                                            poz_de_inceput(I1, 0, Lista, 0, 0, R1, C1), poz_de_inceput(I2, 0, Lista, 0, 0, R2, C2), 
                                                            minmin(Lung1, Lung2, Lung), minmin(R1, R2, R_min), minmin(C1, C2, C_min),
                                                            calculare_poz(Poz1, Poz2, R_min, C_min, Lung, R1, R2, C1, C2).

calculare_poz(Poz1, Poz2, R, C, Lung) :- R_a is R1 - R, R_b is R2 - R, Poz1 is max(R_a, R_b), C_a is C1 - C, C_b is C2 - C, Poz2 is max(C_a, C_b).


minmin(Lung1, Lung2, Lung) :- Lung is min(Lung1, Lung2).                                           

poz_de_inceput(I, [Head | Lista], R, C) :- member(((R, C), [(I, _, _ )]), Lista); member(((R, C), [(I, _, _), _]), Lista);
                                           member(((R, C), [_, (I, _, _)]), Lista).

% poz_de_inceput(I, Ok, Lista, R_Acc, C_Acc, R, C) :- Ok =:= 0, \+ member(((R_Acc, C_Acc), (I, _, _)), Lista), NewR_Acc is R_Acc + 1,
%                                                   poz_de_inceput(I, 1, Lista, NewR_Acc, C_Acc, R, C).
%                                                
%
% poz_de_inceput(I, Ok, Lista, R_Acc, C_Acc, R, C) :- Ok =:= 1, \+ member(((R_Acc, C_Acc), (I, _, _)), Lista), NewC_Acc is C_Acc + 1,
%                                                    poz_de_inceput(I, 0, Lista, R_Acc, NewC_Acc, R, C).
%
%
% poz_de_inceput(I, Ok, Lista, R_Acc, C_Acc, R_Acc, C_Acc) :- member(((R_Acc, C_Acc), (I, _, _)), Lista).
